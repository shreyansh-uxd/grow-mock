...
interface TopIntradayProps {
  onSelectStock: (stock: Stock) => void;
}

export default function TopIntraday({ onSelectStock }: TopIntradayProps) {
  const { topIntraday } = useStocks();

  return (
    <section className="py-3.5 px-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-bold text-slate-800">Top Intraday</h2>
