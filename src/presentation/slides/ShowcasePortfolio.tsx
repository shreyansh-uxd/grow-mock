...
const ShowcasePortfolio = forwardRef<SlideHandle>(function ShowcasePortfolio(_, ref) {
    ...
    <div ref={sceneRef} className="pres-scene justify-center">
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-10 md:gap-14 px-6 md:px-12">
        ...
        <div className="flex-1 flex flex-col items-start text-left max-w-xl space-y-4">
          ...
          >
            Track Net Worth &amp; Automated SIP Growth.
          </div>

          {/* Subtitle */}
          <p ref={subRef} className="text-left text-slate-500 font-normal m-0 text-xs md:text-sm leading-relaxed max-w-lg">
            Monitor portfolio holdings, net return value, daily gainers, and automated stock SIP compounding.
          </p>

          {/* Stacked Feature Callout Markers */}
          <div className="space-y-3 w-full pt-3">
            <div ref={marker1Ref} className="p-3.5 rounded-2xl bg-white border border-slate-200/90 shadow-2xs flex items-center gap-3.5 transition-all hover:border-purple-500/50">
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-900">Total Net Worth Summary Card</h4>
                <p className="text-[11px] text-slate-500">Real-time portfolio valuation &amp; day's P&amp;L returns</p>
              </div>
            </div>

            <div ref={marker2Ref} className="p-3.5 rounded-2xl bg-white border border-slate-200/90 shadow-2xs flex items-center gap-3.5 transition-all hover:border-purple-500/50">
              <div className="w-10 h-10 rounded-xl bg-purple-50 text-purple-700 border border-purple-200/60 flex items-center justify-center shrink-0">
                <Repeat className="w-5 h-5 stroke-[2.2]" />
              </div>
              <div>
                <h4 className="text-xs font-bold text-slate-900">Automated Stock SIP Tracker</h4>
                <p className="text-[11px] text-slate-500">Monthly auto-debit wealth compounding schedule</p>
              </div>
            </div>

            <div ref={marker3Ref} className="p-3.5 rounded-2xl bg-white border border-slate-200/90 shadow-2xs flex items-center gap-3.5 transition-all hover:border-purple-500/50">
              <div className="w-10 h-10 rounded-xl bg-purple-50 text-purple-700 border border-purple-200/60 flex items-center justify-center shrink-0">
                <ShieldCheck className="w-5 h-5 stroke-[2.2]" />
              </div>
              <div>
                <h4 className="text-xs font-bold text-slate-900">Depository Sync &amp; Verification</h4>
                <p className="text-[11px] text-slate-500">Direct CDSL/NSDL verification with zero maintenance</p>
              </div>
            </div>
          </div>
